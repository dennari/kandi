...
Ongelmana on kuitenkin tällaisten sääntöjen määrittämisen vaikeus.
...
nimellä \emph{Gaussin kvadratuuri} \cite{Gauss1814,Gautschi1997}. Kyseessä on elegantti kvadratuuri, jossa hyödynnetään \emph{ortogonaalisia
...
$\lim_{k\to\infty}\vert P_k \vert = 0$. Nyt jos mikä tahansa näin muodostettu Riemann--summien sarja suppenee, eli \emph{konvergoituu},
...
painofunktiota kutsutaan \emph{jakaumaksi} ja $\defint{a}{b}{f(x)w(x)}{x}$ on tällöin $f(x)$:n painotettu keskiarvo.
...
se on ainutkertainen ja $m\leq n$.\cite[s. 128]{Cheney2007}
...
Havaitaan, että mikäli kaavassa \ref{eq_newton_cotes} valitaan $n=1$, ja sovelletaan sitä paloittain,
...
\begin{equation} 
	\label{eq_simpson}
	\defint{a}{b}{f(x)}{x} \approx \frac{h}{3}\left(f(a)+4f(\frac{a+b}{2})+f(b)\right) \\
\end{equation}

Virheen $E_n(p)$ pienentämiseksi voidaan nähdä kaksi eri lähestymistapaa: joko käytetään pienen $n$:n 
Newton-Cotes kaavaa aina vain lyhyemmillä osaväleillä tai käytetään pidempiä osavälejä mutta suurta $n$:ää. 
Ensimmäisessä tapauksessa evaluointipisteiden määrä kasvaa nopeasti epäkäytännöllisen suureksi. 
...
jatkuville funktioille. Toisessa tapauksessa $n:än$ kasvaessa suureksi, voivat painot $w_i$ olla 
...
\begin{table}[h!]
	...
	\begin{tabular}{cccc}
		...
		\midrule
		$P_n$ & Legendre &$1$ & $[-1,1]$ \\
		$P_n^{(\alpha,\beta)}$ & Jacob & $(1-t)^\alpha(1+t)^\beta$ & $[-1,1]$ \\
...
mielivaltaisella tarkkuusteella (vakiopainofunktiolle).
...
\emph{kahden} yhtälön avulla. 

Olkoon $X$ kubatuurisäännön $Q$ tukipisteiden joukko. 
...
samat painot. Kubatuuria sanotaan \emph{keskipisteensä suhteen symmetriseksi} jos pätee
\[
	Q[\v{x^\alpha}]=0\quad\forall \v{\alpha}\in\field{N}_0^n,\;\sum_{i=1}^n\alpha_i=\text{pariton}\;\cite{Lu2005}.
\] 
...
säännössä on 20 pistettä, mutta minimisäännössä, joka on invariantti rotaation suhteen, on 17 \cite{Cools1997}. 
...
sitä ei ole mahdollista saavuttaa. Ne harvat tapaukset joissa tämä alaraja on saavutettu
...
kubatuurisäännön tarkkuusaste $d=2s+1,\;s\in\field{N}_0$ ja olkoon
$R_{2k}^n$ polynomiavaruus, johon kuuluu kaikki tarkkuusasteen $2k$ parilliset
polynomit ja  $R_{2k+1}^n$ polynomiavaruus, johon kuuluu kaikki tarkkuusasteen $2k+1$ parittomat
polynomit. Tällöin pisteiden määrälle $N$ pätee
\begin{equation}
	N\geq 2\dim[R_{s-1}^n]-
	\begin{cases}
		1& \text{jos s on parillinen,}\\
	 	0& \text{jos s on pariton.}
	\end{cases}
	\label{eq_moller1}	 
\end{equation}
Ilmaistuna eksplisiittisemmin kaavasta \eqref{eq_moller1} saadaan:
\begin{equation}
	N\geq \binom{n+s-1}{n}+
	\begin{cases}
		\sum_{k=1}^{n-1}2^{k-n}\binom{k+s-1}{k}& \text{jos s on parillinen,}\\
	 	\sum_{k=1}^{n-1}(1-2^{k-n})\binom{k+s-2}{k}& \text{jos s on pariton \cite{Lu2005}.}
	\end{cases}
	\label{eq_moller2}	 
\end{equation}
Möllerin alarajaa on havainnollistettu kuvassa \ref{fig_mollerpoints},
jossa alarajan mukainen pistemäärä on kuvattu ulottuvuuden $n$
ja tarkkuusasteen $d=2s-1$ funktiona.
\scalefig{fig_mollerpoints}{1.0}{Möllerin alaraja keskipisteensä
suhteen symmetrisille kubatuureille ulottuvuuden ja tarkkuusasteen funktiona}
Kuten kuvasta \ref{fig_mollerpoints} nähdään, lähtee
vaadittavien pisteiden määrä jyrkkään nousuun suurin piirtein 
kun tarkkuusaste ja ulottuvuuksien määrä on ylittänyt kymmenen.
Jos tarkkuusaste pidetään melko matalana, niin pisteiden
määrä ei kasva liian suureksi edes suurilla $n$:än arvoilla.
Esimerkiksi kun $n=100$, saadaan pisteiden
vähimmäismääräksi tarkkuusasteilla $d=5$ ja $d=7$
vastaavasti $N=10101$ ja $N=343600$. Hinrichs \emph{et al} ovat
vastikään johtaneet kubatuurit, jotka saavuttavat
näissä tilanteissa arvot $N=10701$ ja $N=404001$ 
(integroimisalueena n-kuutio ja vaatimuksena symmetrinen 
tulomuotoinen painofunktio) \cite{Hinrichs2007}. 
Nämä ovat ovat jo hyvin lähellä minimiä.
Möllerin alarajan perusteella voidaan joka tapauksessa päätellä,
että suuren dimension ja tarkkuuasteen ongelmissa ratkaisua
täytyy etsiä muualta kuin interpolatorisista kubatuureista.  
...
jossa on vain $6$ pistettä. Yleisille integroimisalueille, kuten neliölle ja vakiopainofunktiolle, 
...
ja painofunktioksi vakio $1$, niin ratkaisemalla edellä esitellyt yhtälöt saadaan ortogonaalisiksi
polynomeiksi lopulta 
\begin{align*}
	K_1=&x^3-\frac{3}{5}x\\
	K_2=&xy^2-\frac{1}{3}x\\
	K_3=&x^2y+y^3-\frac{14}{15}y.
\end{align*}
Näiden nollakohdista saadaan ratkaistua kubatuurin pisteet, jonka jälkeen
painot saadaan pisteiden sijoittamisen jälkeen lineaarisesta yhtälöryhmästä \eqref{eq_nonlinear_equations}.
Lopullinen kubatuuri on \cite[s. 246]{Stroud1971}:
...
Koeasetelmassa keskitytään approksimoimaan kahden muuttujan
funktion integraalia
\begin{equation}
	I_1[f(x,y)]=\int_{C_2}f(x,y)\dd x \ds y. 
\end{equation} 
Integrandeina käytetään funktioita
\begin{align*}
	F_1(x,y)=&e^{-\frac{1}{2(1-0.7^2)}\left(x^2+y^2-2*0.7*xy)\right)}\\
	F_2(x,y)=&\sqrt{\vert xy \vert},
\end{align*}
joita on havainnollistettu kuvassa \ref{fig_3dfunc}. $F_1$ on skalaarikerrointa
lukuunottamatta kaksiulotteinen normaalijakauma, jonka odotusarvo on origo ja jonka
muuttujien välinen korrelaatio on $0.7$. Se on \emph{sileä} funktio, joten polynomiapproksimaatioihin
perustuvien kubatuurien konvergenssin pitäisi olla nopea. Mikäli muuttujien välillä ei olisi
korrelaatiota, voisi tällaisen integrandin integroida yksiulotteisten integraalien tulona.
 
$F_2$ taas valittu siten, että sen osittaisderivaatoilla on epäjatkuvuuskohtia. Tällaisen
integrandin polynomiapproksimaation konvergenssi on huomattavasti hitaampi kuin sileän
funktion tapauksessa. Todellisuudessa tässä tilanteessa kannattaisi hyödyntää
integraalin symmetriaa jakamalla integraali neljään identtiseen osaan, jolloin näiden epäjatkuvuuskohtien yli ei tarvitsisi integroida
lainkaan. Nyt on kuitenkin pyritty konstruoimaan kubatuurien kannalta
haastava tilanne ja integraalia ei jaeta osiin.

\scalefig{fig_3dfunc}{1.00}{Ensimmäisen koetilanteen testifunktioiden kuvaajat. Integroimisaluetta $[-1,1]^2$ 
on pyritty havainnollistamaan läpinäkyvällä suorakulmaisella särmiöllä. Integraalin arvo
on särmiön ja funktion kuvaajan rajaama tilavuus. }


Kubatuureiksi valitaan seuraavat menetelmät:
\begin{itemize}
  \item $Q_{3^2}:=9$-pisteinen tulosääntö, joka perustuu aiemmin
  johdettuun $3$-pisteiseen Gaussin kvadratuuriin
  \item $Q_{7}:=7$-pisteinen Radonin minimisääntö
  \item $Q_{10^2}:=100$-pisteinen tulosääntö, joka perustuu $10$-pisteiseen Gaussin kvadratuuriin
  \item $Q_{68}:=68$-pisteinen rotaatioinvariantti korkean tarkkuusasteen kubatuuri.
\end{itemize}
$Q_{3^2}$:n yleinen tarkkuusaste on $5$, kuten myös $Q_{7}$:n tarkkuusaste. 
$Q_{10^2}$:n yleinen tarkkuusaste on $19$. $Q_{68}$:n tarkkuusaste on $19$ 
ja se on johdettu aiemmin esiteltyä invarianttimenetelmää käyttäen \cite{Omelyan2006}. 
Sen tukipisteiden määrä on myös pienin tunnettu sarjassaan. Kunkin kubatuurin
käyttämät tukipisteet on merkitty kuvaan \ref{fig_points}, jossa pisteen
tummuus vastaa sen saamaa painoa (mitä tummempi, sitä suurempi paino). 
$Q_{10^2}$:n ja $Q_{10^2}$:n pisteiden jakaumat 
heijastavat niiden tulomuotoa. Selvästi on myös havaittavissa, että
pisteiden jakauma on tiheämpi ja painot suuremmat alueen reunoilla kuin sen keskellä.

\scalefig{fig_points}{0.7}{Koasetelmassa käytettyjen kubatuurien tukipisteet integroimisalueessa $C_2$. Pisteen tummuus vastaa
sen saamaa painoa.}

\section{Tulokset}

Koeasetelman tulokset on esitetty taulukossa \ref{table_results1}.
Siihen on lisäksi laskettu suhteellinen virhe $E[f]$, joka on saatu vertaamalla
kubatuurin tulosta oikeaan tulokseen. Ensimmäisen integrandin tapauksessa
on hyödynnetty MATLAB(R):in $mvncdf$ funktiota, joka pystyy takaamaan
annetun virhetoleranssin. Toiselle integrandille taas
on käytetty MATLAB:in funktiota $dblquad$, joka on tulomenetelmä ja 
perustuu kvadratuurifunktion $quad$ \cite{MatlabQuad}. Myös $dblquad$ pystyy
takaamaan annetun virhetoleranssin.
...
Tulosten perusteella kaikki neljä kubatuurisääntöä suoriutuivat
$F_1$:n integroimisesta hyväksyttävästi. $Q_7$:n $2.6\%$:n virhe on tosin
melkein $9$ kertaa suurempi kuin toiseksi suurin virhe. Tämä johtuu
todennäköisesti siitä, että $Q_{3^2}$ integroi tarkasti
hieman suuremman joukon polynomeja kuin $Q_7$ ($Q_{3^2}$:n \emph{yleinen} tarkkuusaste
on sama kuin $Q_7$:n tarkkuusaste). Esimerkiksi $Q_{3^2}[x^5y^5]=0$, mutta
$Q_{7}[x^5y^5]\neq 0$. Kuten oli odotettavissa, $Q_{10^2}$ ja $Q_{68}$ saavuttivat
erittäin tarkan tuloksen.  Se, että $Q_{10^2}$:n absoluuttinen virhe oli
luokkaa $10^{-13}$ ja $Q_{68}$:n luokkaa $10^{-9}$ johtunee myös tässä
tapauksessa tulosäännön yleisestä tarkkuusasteesta.

$F_2$ tuotti alhaisen tarkkuusasteen kubatuureille $Q_{3^2}$ ja $Q_7$
huomattavia ongelmia. Kummankaan tulos ei ole käyttökelpoinen, mutta
$Q_{3^2}$:n virhe on todella suuri. Tämän täytyy johtua tukipisteiden
sijoittumisesta epäedullisesti $F_2$:n integroinnin suhteen.
Korkean tarkkuusasteen säännöt saavuttavat hankalasta integrandista 
huolimatta melko hyvän tuloksen ja tulosäännön hiukan pienempi 
virhe johtunee jälleen sen yleisestä tarkkuusasteesta. 
Kuten aiemmin jo todettiin, parempiin
tuloksiin pääsemiseksi tulisi hyödyntää integraalin symmetriaa.

\section{Yhteenveto}

Yhden muuttujan funktioiden numeerinen integrointi on ongelma,
johon on tarjolla tehokkaita menetelmiä. 
Annetulla tukipisteiden määrällä ovat
Gaussin kvadratuurit näistä polynomiapproksimaation
mielessä tarkimpia. Niiden käyttäminen vaatii kuitenkin
painojen selvittämisen lisäksi myös tukipisteiden laskemisen, joka
kuitenkin onnistuu ratkaisemalla tiettyjen ortogonaalisten
polynomien nollakohtia.
Useimmissa tapauksissa riittää niinkin yksinkertainen
keino kuin Simpsonin menetelmä, jonka adaptiivista versiota
käyttää esimerkiksi MATLAB:in kvadratuurifunktio $quad$ \cite{MatlabQuad}.
Konvergenssi on taattu, jos integroimisväli vain jaetaan tarpeeksi
pieniin osiin ja huomioidaan mahdolliset derivataan epäjatkuvuuskohdat.

Tilanne muodostuu huomattavasti ongelmallisemmaksi muuttujien määrän,
eli integraalin dimension, kasvaessa. Tällöin erilaisia
integroimisalueita on ääretön määrä. Mikäli integroimisalue 
(ja painofunktio) on mahdollista
saattaa tulomuotoon, voidaan ainakin kahdessa ja
kolmessa dimensiossa hyvin käyttää yhden dimension
menetelmään, yleensä Gaussin kvadratuuriin, perustuvaa tulosääntöä.
Tulosääntöjä käytettäessa tarvittavien pisteiden, ja siis funktioevaluaatioiden,
määrä kasvaa eksponentiaalisesti dimension suhteen. Tätä kutsutaan
dimensionaalisuuden kiroukseksi.

Dimensionaalisuuden kirousta on mahdollista paeta käyttämällä
interpolatorisia kubatuureja. Tällöin vaadittavien funktioevaluaatioiden
määrä ei enää riipu eksponentiaalisesti dimensiosta. Näiden menetelmien
suurin ongelma on kuitenkin niiden määrittäminen. Gaussin kvadratuurin idea
voidaan muotoilla myös monessa ulottuvuudessa, mutta analogisen
kubatuurin olemassaolo vaatii, että integroimisalueesta ja painofunktiosta
riippuvilla moniulotteisilla ortogonaalisilla polynomeilla
on tarpeeksi suuri määrä yhteisiä nollakohtia. Tiedetään, että näin
ei ole tavallisten integroimisalueiden, kuten neliön tai kolmion,
ja vakiopainofunktion tapauksessa.

Vaikka tavallisesti ei voidakaan saavuttaa Gaussin kvadratuurien
mielessä optimaalista pisteiden määrää, interpolatorinen kubatuuri
on kuitenkin olemassa jokaiselle integraalille. Sen määrittäminen
voi kuitenkin olla äärimmäisen vaikeaa, sillä se vaatii
suuren epälineaarisen yhtälöryhmän ratkaisemista. Vaikeutta lisää myös 
se, että yleisessä tapauksessa vaadittavien pisteiden vähimmäismäärää ei tiedetä
ja jotta yhtölöryhmä voitaisiin edes muodostaa, täytyisi pistemäärälle
saada jokin arvio. Tilannetta voidaan helpottaa hyödyntämällä approksimoitavan
integraalin symmetriaa. Pakottamalla etsittävälle kubatuurisäännölle tietty
symmetria voidaan yhtälöryhmää huomattavasti pienentää ja sen jälkeen
se voidaan yrittää ratkaista hyödyntämällä tietokoneiden suurta
laskentatehoa \cite[s. 6]{Cools2002}. Tyypillinen esimerkki tällaisesta
proseduurista löytyy lähteestä \cite{Stoyanova2009}. Kattava tietokanta
julkaistuista interpolatorisista kubatuureista on pyritty muodostamaan lähteessä \cite{Cools2003}. 

Vaikka interpolatoristen kubatuurien ratkaiseminen olisi helpompaa,
on selvää että joka tapauksessa vaadittavien pisteiden
määrä nousee liian suureksi jos dimensioita on muutamaa
kymmentä enemmän. Tällöin täytyy turvautua täysin eri typpisiin
menetelmiin, kuten erilaisiin Monte-Carlo menetelmiin. Suuridimensioisia
integraaleja käsitellään esimerkiksi lähteissä \cite{Cools2009,Cools2010}. 
