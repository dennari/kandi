...
\end{equation}
ja sen yleiselle tarkkuusasteelle $d$ pätee $\min\left\{d_r,d_s\right\} < d < \max\left\{d_r,d_s\right\}$ \cite{Cools1997}.
...
\begin{equation}
	 ...
	 \end{array}
\end{equation} Yhtälöryhmälle on aina olemassa ratkaisu,
mikäli pisteet $\v{x}_i$ eivät sijaitse samalla $d$-asteisella ``käyrällä'' \cite{Cools2001}. 

Seuraa tuttu kysymys: kuinka pisteet $x_i$ tulisi sitten valita? Yksiulotteisessa interpolatorisessa 
tapauksessa $N=m+1$ ja havaittiin, että pisteet on mahdollista valita siten, että
osa painoista häviää. Päästiin myös selkeään lopputulokseen: säilyttääkseen tarkkuusasteen
$m$, kvadratuurin painoista voitiin ``hävittää'' maksimissaan puolet ja saatiin Gaussin kvadratuuri.

Kaikkien mahdollisuuksien käyttämiseksi, annetaan myös kaikkien pisteiden olla muuttujia.
Yleisesti ottaen, jokainen piste $\v{x}_k$ tuo yhtälöryhmään $n+1$ muuttujaa:
koordinaatit $x_{1,k},\dots,x_{n,k}$ ja painon $w_k$, joten $N$-pisteisen
kubatuurin yhtälöryhmässä on $N(n+1)$ muuttujaa. Jotta yhtälöryhmälle voitaisiin kuvitella
olevan ratkaisu, olisi lupaavaa jos muuttujia olisi vähintään yhtä paljon kuin
yhtälöitä $N(n+1)\geq\binom{n+d}{d}$, josta saadaan pisteiden määrälle alaraja $N\geq\frac{(n+d)!}{(n+1)!d!}$.
Näin ollen esimerkiksi $n=20$ dimensioisen ja $d=3$ tarkkuusasteisen kubaatuurin selvittäminen
vaatisi $\frac{23!}{20!3!}=1771$ yhtälöisen systeemin ratkaisua ja siinä tulisi olla vähintään  
$N\geq\frac{23!}{21!3!}=85$ pistettä.

Yhtälöryhmän \eqref{eq_nonlinear_equations}
ratkaisemiseksi voidaan nähdä kaksi eri lähestymistapaa, joista kumpikin johtaa
omanlaiseensa teoriaan kubatuurisääntöjen muodostamiseksi:
\begin{enumerate}
  \item Yritetään ratkaista yhtälöryhmä suoraan
  \item Yritetään löytää (ortogonaalisia)polynomeja, jotka häviävät kubatuurin tukipisteissä 
\end{enumerate}

\subsubsection{Invarianttiteoria}

\subsubsection{Ideaaliteoria}


\subsection{Kolmipisteinen Gauss-Hermite kubatuuri}
