...
samansuuruisiksi suorakulmioiksi. Matemaattinen konstruktio, jolla on läheinen yhteys tähän ongelmaan 
on \emph{integraali}· Tänä päivänä tekniikan alan ammattilaiset arkkitehdeistä tilastotieteilijöihin 
...
\emph{Gaussisia kvadratuureja} sovelletaan useassa ulottuvuudessa. Gaussisten kvadratuurien erinomaisuudesta seuraa
luonnollisesti kysymys siitä, voidaanko sen periaatteita soveltaa usean ulottuvuuden integraalilaskusääntöjen, 
\emph{kubatuurien}, muodostamisessa ja jos voidaan, kykenevätkö ne yhtä hyvään suoritukseen. 
 
Tässä työssä käydään ensin  läpi tarvittavat esitiedot (kuten Riemann integraali ja polynominen approksimaatio), 
jotta voidaan esittää Gaussisten kvadratuurien, siis yhden ulottuvuuden numeeristen integrointisääntöjen, idea.
Tämän jälkeen luodaan katsaus usean ulottuvuuden numeeriseen integrointiin, keskittyen Gaussisten kvadratuurien
yleistämiseen kubatuureiksi (on myös muita usean ulottuvuuden numeerisia integrointimenelmiä, kuten Monte-Carlo menetelmät). 
...
puolisuunnikasmenetelmälle löytyy ilmeinen parannusehdotus: käytetään approksimointiin korkeamman asteen polynomia.\cite{Gautschi1997}
...
seuraavan ongelman: etsi polynomi $p$, jolle pätee $p(x_i) = y_i\quad\forall i$, kun $\{x_i\}$ on $n+1$ 
erillistä pistettä ja $\{y_i\}\in\R$ niitä vastaavat mielivaltaiset arvot. Osoittautuu että, $p$ on aina olemassa, 
se on ainutkertainen ja sen asteluku on korkeintaan $n$.\cite{Cheney2007}
...
poikkeavat toisistaan siinä, montako pistettä interpolointiin käytetään ja ovatko
välin päätepisteet mukana (suljettu menetelmä) vai eivät (avoin menetelmä).\cite{Gautschi1997}
