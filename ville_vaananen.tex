...
\begin{table}[h!t]
	...
	\centering
	\begin{tabular}{cccc}
		Merkintä & Nimi & Määritelmä & $w(\v{x})$ \\ \hline
		...
		$E_n^{r^2}$ & koko n-avaruus & $-\infty < x_i < \infty$ & $e^{-\v{x}^T\v{x}}$ \\
		$E_n^{r}$ & koko n-avaruus & $-\infty < x_i < \infty$ & $e^{-\sqrt{\v{x}^T\v{x}}}$
	\end{tabular}	
	\label{table_regions}
...
ja painofunktioille. Mikäli tulosääntö on mahdollista muodostaa, on sen löytäminen helppoa ja virhearviokin
...
Interpolatoriset kvadratuurisäännöt, kuten Newton--Cotes kvadratuurit, voitiin muodostaa
helposti integroimalla interpolaatiopolynomia. Interpolaatiopolynomille ei valitettavasti
...
Aina voidaan kuitenkin vaatia, että kubatuurin on oltava tarkka kaikille  $p(\v{x})\in\field{P}_n^d$,
mikä tarkoittaa että sen oltava tarkka jollekin $\field{P}_n^d$:n kannalle. Tällöin
\begin{equation}
	Q[f_i]=I[f_i],\quad i=1,\dots,\dim[\field{P}_d^n]=\binom{n+d}{d}=\frac{(n+d)!}{n!d!}
	\label{eq_nonlinear_equations} 
\end{equation}
on epälineaarinen yhtälöryhmä, missä $f_i$:t muodostavat $P_n^d$:n kannan ja muuttujina
ovat tukipisteet ${\v{x_i}}$ ja painot ${w_i}$. Yhtälöryhmän \eqref{eq_nonlinear_equations}
