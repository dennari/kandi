...
On olennaista huomata, että ratkaisun helpottumista lukuunottamatta ei ole mitään erityistä syytä
miksi kubatuurisäännön symmetrian pitäsi olla sama kuin integroimisalueen tai painofunktion.
Päinvastoin, pakottamalla kubatuurisääntö noudattamaan tiettyä rakennetta voidaan
...
säännössä on 20 pistettä, mutta \emph{minimisäännössä}, joka on invariantti rotaation suhteen, on 17 \cite{Cools1997}. 
...
kaavan \eqref{eq_general_lower_bound} alaraja on sama tarkkuusasteille $d=2k$ ja $d=2k+1$.
...
että suuren dimension ja tarkkuusasteen ongelmissa ratkaisua
...
$P^3$:lla normalisoitua polynomia jonka asteluku on täsmälleen $3$ ja $Q$:lla
...
asteen ortogonaalipolynomi. Jos nyt määritellään vakiot
\[ 
	\begin{array}{rcl}
...
\end{equation} 
jossa siis integroimisalueena on neliö ja painofunktio $w(x)=1$.
Integrandeina käytetään funktioita
\begin{align*}
	F_1(x,y)=&e^{-\frac{1}{2(1-0.7^2)}\left(x^2+y^2-1.4xy)\right)}\\
...
muuttujien välinen korrelaatio on $0.7$. Se on sileä funktio, joten polynomiapproksimaatioihin
...
$F_2$ taas on valittu siten, että sen osittaisderivaatoilla on epäjatkuvuuskohtia. Tällaisen
...
integraalin symmetriaa jakamalla integraali neljään identtiseen osaan, jolloin osittaisderivaattojen 
epäjatkuvuuskohtien yli ei tarvitsisi integroida
...
Sen tukipisteiden määrä on myös pienin tunnettu sarjassaan. $Q_{3^2}$:n ja
$Q_{10^2}$ laskemiseksi käytettiin jo aiemmin mainittua MATLAB-funktiota,
jonka perusteella muodostettiin kaksiulotteinen tulosääntö. 
Funktio on esitetty liitteessä \ref{appendixA}.    

Kunkin kubatuurin käyttämät tukipisteet on merkitty kuvaan \ref{fig_points}, jossa pisteen
...
on hyödynnetty MATLAB:in $mvncdf$ funktiota, joka pystyy takaamaan
annetun virhetoleranssin. Toiselle integrandille taas
on käytetty MATLAB:in funktiota $dblquad$, joka on tulomenetelmä ja 
perustuu kvadratuurifunktioon $quad$ \cite{MatlabQuad}. Myös $dblquad$ pystyy
...
on sama kuin $Q_7$:n tarkkuusaste). Esimerkiksi $E\big[Q_{3^2}[x^5y^5]\big]=0$, mutta
$E\big[Q_{7}[x^5y^5]\big]\neq 0$. Kuten oli odotettavissa, $Q_{10^2}$ ja $Q_{68}$ saavuttivat
...
jonka ratkaisemiseksi on tarjolla tehokkaita menetelmiä. 
Annetulla tukipisteiden määrällä ovat
Gaussin kvadratuurit näistä polynomiapproksimaation
kannalta tarkimpia. Niiden käyttäminen vaatii kuitenkin
...
ja jotta yhtälöryhmä voitaisiin edes muodostaa, täytyisi pistemäärälle
...
\section[Liite]{MATLAB-funktio\label{appendixA}}


\lstinputlisting[label=fun_trefethen2D]{/Users/dennari/Documents/MATLAB/gauss2D.m}
